 ...
 */
export function getLanguageContentPath(locale: string = "en"): string {
 ...
 */
export function isGuideFrontmatter(
    data: unknown
): data is import("../types").GuideFrontmatter {
...
export function isAlternativesFrontmatter(
    data: unknown
): data is import("../types").AlternativesFrontmatter {
...
export function isServiceFrontmatter(
    data: unknown
): data is import("../types").ServiceFrontmatter {
 ...
 */
export function isCategoryMetadata(
    data: unknown
): data is import("../types").CategoryMetadata {
...
export function extractContentSegments(content: string): ContentSegments {
    ...
    const newSectionRegex = /<!-- section:(\w+) -->\s+([\s\S]*?)<!-- end-section -->/g;

    // Define regex for the legacy section format
    // ---section:name ... ---
    const legacySectionRegex = /---section:(\w+)\s+([\s\S]*?)---(?=\s*(?:---section:|\s*$))/g;
 ...
 */
export function extractStepsWithMeta(
    content: string
): Array<{
    title: string;
    id: string;
    ...
    const stepRegex = /<!-- step-start -->\s+<!-- step-meta\s+([\s\S]*?)-->\s+([\s\S]*?)<!-- step-end -->/g;
    const steps = [];
    let match;
...
export function extractMigrationSteps(
    content: string,
    segments?: ContentSegments
): Array<{ title: string; id: string; completionMarkers?: string[]; video?: string; }> {
